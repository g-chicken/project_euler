...
fn main() {
    ...
    let num: [u32; 120];
    let first_child_index: [usize; 120];
...
fn read_file() -> Result<([u32; 120], [usize; 120]), Box<dyn std::error::Error>> {
    let mut num: [u32; 120] = [0; 120];
    let mut child_index: [usize; 120] = [0; 120];
    let mut row = 1;
    let mut index = 0;

    let buf = BufReader::new(File::open("./prob18.txt").expect("open failed"));
